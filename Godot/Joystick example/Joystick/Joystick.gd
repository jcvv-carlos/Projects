...
func _input(event):
	...
	if event is InputEventMouseButton && event.button_index == BUTTON_LEFT && event.is_pressed() == true && estado==false:
		#Si se presiona el Joystick procede al arrastre
		#Hay que corregir esta parte!!!!
		if event.position > __ctrl.get_global_position() && event.position < __ctrl.get_global_position() + __ctrl.get_size():
			estado = true
	
	#Si el boton izquierdo del mouse se suelta
	elif event is InputEventMouseButton && event.button_index == BUTTON_LEFT && event.is_pressed() == false && estado==true:
		#Resetea la posicion del Joystick al soltarlo
		__ctrl.set_position(__centro)
		__h=0
		__v=0
		estado = false
	#Si se comienza el arrastre del Joystick
	elif event is InputEventMouseMotion && estado==true:
		#Obtiene la posicion centrada del Joystick dentro del Control 
		var pos = Vector2(get_local_mouse_position() - (__ctrl.get_size()/2))
